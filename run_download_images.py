...
def sent_image_search(date_t: ee.Date, zone: ee.Geometry, sent: int, optparam1: dict, i: int, opt_search: str) -> Tuple[
    int, ee.ImageCollection]:
    """
...
def get_sentinel1_image(date_t, zone, optparam1, opt_search="both", sent=1):
    ...
    all_found, final_image_collection = get_biggest_s1_image(zone,
                                                             dayli_collection)  # boolean wether or not all the good image has been selected
...
def download_sent2_sent1(bd, ed, zone, sent2criteria, optparam1, ccp, name_s2):
    ...
    if name_s2 is None:  # Only for automatic search, should be improved and tested
        ...
        for sub_col in list_subcol_sent2_t1:  # Go over all the different subcollection
            ...
            list_name_sent1, list_date_sent1 = get_sentinel1_image(date1_sent2_subcol,
                                                                   zone_sent2.intersection(zone, 0.001), optparam1,
                                                                   "both")
        ...
        dict_image_dwnld2.update({name: eedate_2_string(date1_sent2_subcol)})
        list_name_sent1, list_date_sent1 = get_sentinel1_image(date1_sent2_subcol, zone_sent2.intersection(zone, 0.001),
                                                               optparam1, "both")
        dict_image_dwnld1.update(
            dict(zip(list_name_sent1, [eedate_2_string(date) for date in list_date_sent1])))
