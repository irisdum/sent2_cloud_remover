...
for F in $(ls -1d "${sourceDirectory}"/S2*.SAFE); do
  ...
  while IFS= read -r poly; do
    ...
    ${gptPath} ${graphXmlPath} -e -p "${parameterFilePath}"  -Pfile="${targetDirectory}/b3_$(removeExtension "$(basename ${F})")_prepro_${i}" -PBands="B3" -Pgeometry="${poly}" -t  ${targetFile} ${sourceFile}
    ${gptPath} ${graphXmlPath} -e -p "${parameterFilePath}"  -Pfile="${targetDirectory}/b4_$(removeExtension "$(basename ${F})")_prepro_${i}" -PBands="B4" -Pgeometry="${poly}" -t  ${targetFile} ${sourceFile}
